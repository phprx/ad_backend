...
class Q13score:
    ...
    def getScore(self):
        ...
        similarity = difflib.SequenceMatcher(
            None, str(patient_ans['year']), str(correct_ans['year'])).quick_ratio()
        ...
        # 判断日
        similarity = difflib.SequenceMatcher(
            None, str(patient_ans['day']), str(correct_ans['date'])).quick_ratio()
        self.score += (1 if similarity > 0.5 else 0)
        # 判断周几
        similarity = difflib.SequenceMatcher(
            None, str(patient_ans['week']), str(correct_ans['day'])).quick_ratio()
        self.score += (1 if similarity > 0.5 else 0)
        # 判断地点
        similarity = difflib.SequenceMatcher(
